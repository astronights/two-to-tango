  ...
  function solve(pos) {
    ...
    for (const val of [SUN,MOON]) {
      if (isValidWithConstraints(grid,r,c,val,cmap,size)) {
        grid[r][c]=val; solve(pos+1); grid[r][c]=EMPTY;
      }
    }
  }
  solve(0);
...
function generatePuzzle(difficulty) {
  ...
  for (let attempt=0; attempt<20; attempt++) {
    ...
    const pairs=shuffle([
      ...Array.from({length:size},(_,r)=>Array.from({length:size-1},(_,c)=>[r,c,r,c+1])).flat(),
      ...Array.from({length:size-1},(_,r)=>Array.from({length:size},(_,c)=>[r,c,r+1,c])).flat(),
    ]);
    const constraints=pairs.slice(0,numConstraints).map(([r1,c1,r2,c2])=>({
      r1,c1,r2,c2, type:solution[r1][c1]===solution[r2][c2]?'=':'x',
    }));
    const clueGrid=solution.map(row=>[...row]);
    const cells=shuffle(Array.from({length:size},(_,r)=>Array.from({length:size},(_,c)=>[r,c])).flat());
    let removed=0;
    ...
    for (const [r,c] of cells) {
      ...
      clueGrid[r][c]=EMPTY;
      if (countSolutions(size,clueGrid,constraints)!==1) clueGrid[r][c]=saved;
      else removed++;
    }
    if (countSolutions(size,clueGrid,constraints)===1) {
      return {size,solution,constraints,clueGrid,clues:clueGrid.map(row=>row.map(v=>v!==EMPTY))};
    }
  }
  const solution=generateSolution(size);
  return {size,solution,constraints:[],clueGrid:solution.map(row=>[...row]),clues:solution.map(row=>row.map(()=>true))};
}
...
const SUN_ICON = `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <g stroke="currentColor" stroke-width="1.75" stroke-linecap="round">
    ...
    <line x1="5.64" y1="5.64" x2="7.4" y2="7.4"/><line x1="16.6" y1="16.6" x2="18.36" y2="18.36"/>
    <line x1="5.64" y1="18.36" x2="7.4" y2="16.6"/><line x1="16.6" y1="7.4" x2="18.36" y2="5.64"/>
  </g>
</svg>`;
...
const state = {
  size:SIZE, grid:[], solution:[], clues:[], constraints:[], cmap:{},
  difficulty:'medium', errors:new Set(), constraintErrors:new Set(), won:false,
};
...
function findErrors() {
  ...
  const errs=new Set(), cerrs=new Set();
  ...
  for (let i=0; i<constraints.length; i++) {
    ...
    if (v1!==EMPTY&&v2!==EMPTY) {
      if ((type==='='&&v1!==v2)||(type==='x'&&v1===v2)) {
        errs.add(`${r1},${c1}`); errs.add(`${r2},${c2}`); cerrs.add(i);
      }
    }
  }
  state.errors=errs; state.constraintErrors=cerrs;
}
...
function cellSize() {
  ...
  return Math.min(76, Math.floor(avail / (6 + 5 * 0.22)));
}
...
function render() {
  ...
  const tpl = Array.from({length:size*2-1},(_,i)=>i%2===0?`${cs}px`:`${gs}px`).join(' ');
